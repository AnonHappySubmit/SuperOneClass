...
class Visualizer(object):
    ...
    def plot_subgrid(self, images,  filename=None):
        rows, cols = 6, 8
        scale = .75
...
#             ax.imshow((image * 255).astype(np.uint8), interpolation='none', aspect='equal',
#                       cmap=cmap)
...
#                     edgecolor='red', fill=False))
    ...
    def plot(self, epoch, x, mask, bbox, x_recon, x_gen):
        if self.at_start:
            ...
            self.at_start = False
        self.plot_subgrid( x_recon, 
                          self.recons_dir / f'{epoch:04d}.png')
        self.plot_subgrid(x_gen,  self.gen_dir / f'{epoch:04d}.png')
...
#         plt.tight_layout()
#         plt.savefig(str(self.output_dir / 'loss.png'))
#         plt.close(fig)

        if self.print_header:
            logging.info(' ' * 6 + '  '.join(
                f'{key:>10}' for key in sorted(losses)))
            self.print_header = False
        logging.info(f'[{epoch:3}] ' + '  '.join(
            f'{val:10.3f}' for _, val in sorted(losses.items())))